...
import { braveNewsSearchResponseSchema } from './brave-news-search.schema';

export class BraveNewsSearchMapper {
  toSearchResults(body: unknown, query: string): ContentSearchResult[] {
    const parsed = braveNewsSearchResponseSchema.parse(body);
  ...
  toSearchResult(
    ...
    const authorName =
      item.source?.name?.trim() || item.meta_url?.hostname?.trim();
